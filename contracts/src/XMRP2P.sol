...
contract XMRP2P is Ownable {
    ...
    function offer(
        ...
        require(
            0 == parameters.MAXIMUM_OFFER_BOOK_SIZE ||
                nextOfferId <= parameters.MAXIMUM_OFFER_BOOK_SIZE,
        ...
        uint256 evmAmount = offerType == OfferType.BUY
            ? msg.value
            : (msg.value * DEPOSIT_DENOMINATOR) / parameters.DEPOSIT_RATIO;
        uint256 deposit = offerType == OfferType.BUY
            ? ((msg.value *
                parameters.DEPOSIT_RATIO +
                DEPOSIT_DENOMINATOR -
                1) / DEPOSIT_DENOMINATOR)
            : msg.value;

        require(
            evmAmount >= parameters.MINIMUM_OFFER &&
                evmAmount <= parameters.MAXIMUM_OFFER,
            ErrorInvalidAmount()
        );
    ...
    function take(
        ...
        if (offer.kind == OfferType.BUY) {
            ...
            uint256 publicViewingKey = Ed25519.changeEndianness(
    ...
    function cancel(uint256 offerId) public nonReentrant {
        ...
        uint256 amount = offer.kind == OfferType.BUY
            ? offer.amount
            : offer.kind == OfferType.SELL
                ? offer.deposit
                : 0;
    ...
    function quit(
        ...
        require(
            offer.kind != OfferType.SELL || block.number > offer.blockTaken,
        ...
        uint256 amountR1 = offer.kind == OfferType.BUY
            ? offer.amount
            : offer.kind == OfferType.SELL
                ? offer.deposit
                : 0;
        uint256 amountR2 = offer.kind == OfferType.BUY
            ? offer.deposit
            : offer.kind == OfferType.SELL
                ? offer.amount
                : 0;
    ...
    function claim(
        ...
        Offer storage offer = offers[offerId];
        require(
            offer.state == OfferState.READY || offer.state == OfferState.TAKEN,
            ErrorOfferNotReadyOrTaken()
        );
    ...
    function _setParameters(Parameters memory _parameters) internal {
        ...
        require(
            parameters.DEPOSIT_RATIO > 0 &&
                parameters.DEPOSIT_RATIO <= DEPOSIT_DENOMINATOR,
